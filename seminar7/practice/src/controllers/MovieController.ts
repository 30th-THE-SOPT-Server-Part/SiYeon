...
//     try{
//         const data = await MovieService.getMoviesBySearch(search as string); //string으로 타입 단언.
...
//option 유 버전 +pagination.
const getMoviesBySearch = async (req: Request, res: Response) => {
    const { search, option } = req.query; //option도 쿼리 스트링에서 받아와.

    const isOptionType = (option: string): option is MovieOptionType => { //custom 타입 가드임. 
    ...
    try{
        const data = await MovieService.getMoviesBySearch(search as string, option as MovieOptionType, page); //string으로 타입 단언.
        res.status(statusCode.OK).send(util.success(statusCode.OK, message.SEARCH_MOVIE_SUCCESS, data));
